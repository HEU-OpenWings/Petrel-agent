...
必须挡住的输入：`NaN`、`ratio <= 0`、`ratio >= 1`、`cap <= 0`、非整数 cap、
`"1"` / `"yes"` 这类非标准布尔。放过任意一个的后果是**永不压缩或每轮都尝试压缩**，
而且没有任何报错指向配置。新增三项各配一条 config 单测。

空串按未设置处理，返回默认值；这与同文件的 `port()` 不同（`port()` 对空串抛错，
因为 `Number("") === 0` 落在越界范围），是有意的取舍——`.env` 里留空是「未设置」
的惯用写法，不值得为了与 `port()` 一致而让进程为空串启动失败。
...
- **处置**：立刻 `maybeCompact(..., { force: true })`（无视阈值与 cooldown），
  ...
  **文案必须按 `CompactionOutcome` 分支，不能无条件说「已压缩」**：
  ...
  | outcome | 文案 |
  | --- | --- |
  | `compacted`（压完仍未超窗口） | 「上下文超出模型窗口，已自动压缩历史，请重新发送刚才那条消息」 |
  | `compacted`（压完仍然超窗口） | 「单条消息或单轮内容超出模型窗口，压缩无法解决。请缩短输入或换用更大窗口的模型」 |
  | `failed` | 「上下文超出模型窗口，且自动压缩失败。请新建会话继续」——**不带原始 `error.message`**，
    provider SDK 的报错可能含限流阈值、区域信息，个别 SDK 甚至会回显请求 id 或 key 片段；
    原始 error 只进日志（`logger.warn({ err, sessionId }, ...)`），不进用户可见文案 |
  | `skipped`（`nothing-to-compact` / `ineffective` / 其他） | 「上下文超出模型窗口，压缩已无法再回收空间。请新建会话继续」 |

  无条件说「已压缩，请重发」会在摘要限流、单条消息本身超窗口、守卫阻断这三种情况下
  形成**死循环**：用户重发 → 又 overflow → 又被告知已压缩请重发。

  **真实的死循环触发条件不是 `nothing-to-compact`**：`compact()` 恒定保留即将 `prompt`
  的这条消息（不会砍掉最新一轮），所以「压完仍超窗口」这一支才是单条消息本身过大时
  实际会走到的路径；`nothing-to-compact` 经补救压缩这条路径实际不可达（它只在
  `prepareCompaction` 发现分支最后一条已经是 compaction 条目——即上次压缩后再没写过
  任何东西——时抛出，而补救压缩必然发生在 `harness.prompt()` 已经把这一轮消息追加进
  会话树之后，两个条件互斥），归到通用 `skipped` 文案即可。
