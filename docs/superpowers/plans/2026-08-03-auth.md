...
**Goal:** 给 `apps/server` 补上邮箱密码认证（JWT + httpOnly cookie），把硬编码的 `DEFAULT_USER_ID` 换成真实用户，并修掉会话链路上的三处越权。

**Architecture:** 零新增依赖——JWT 用 Hono 内置的 `hono/jwt`，cookie 用 `hono/cookie`，密码哈希用 Node 内置 `node:crypto` 的 `scrypt`。认证代码全部落在 `apps/server`，不新建 package。数据层在 `packages/database` 增加 users repository 并给会话 repository 的三个方法加 `userId` 收窄。
...
## 完成情况（截至 2026-08-03，17.5/18）

代码全部落地，只剩 Task 18 的浏览器与容器人工验收。执行方式是 subagent-driven-development：
每个任务一个实现 subagent，之后一轮 spec + 质量 review，发现进修复轮。
...
| 10 | service 与路由接真实用户，挂上 requireAuth | ✅ | `a409202` + `e38cebf`（修复轮） |
| 11 | 越权回归测试组 | ✅ | `67d934f` |
| 12 | 前端 user store 重写 | ✅ | `55ab589` |
| 13 | 启动恢复登录态与路由守卫 | ✅ | `e7f7c34` |
| 14 | 登录注册页 | ✅ | `064307e` |
| 15 | admin 用户管理页 | ✅ | `67598c6` + `a3f80cc`（侧栏入口） |
| 16 | 清理默认用户与 username 列 | ✅ | `6881f89` |
| 17 | 更新文档 | ✅ | `e16c2a2` |
| 18 | 端到端验收 | ⚠️ 自动化部分已过，浏览器/容器验收待人工 | — |

**当前状态**：前后端都已接通。后端注册/登录/登出/me、admin 端点、`/api/*` 全挂
`requireAuth`、`/api/admin/*` 额外挂 `requireAdmin`、会话按 `user_id` 收窄；前端 cookie
store、启动恢复登录态、路由守卫、邮箱密码登录注册页、admin 用户管理页与侧栏入口都已就位。

Task 18 的 Step 1（`typecheck` / `lint` / `test` / `build`）四项已跑，全部 exit 0
（292 passed | 2 skipped）。Step 2–4 需要在容器与浏览器里人工完成。

### 审查抓到的四个真问题（都已修，记下来避免重蹈）

1. **Task 12**：`logout()` 从同步改异步后，`http.js` 的 401 分支不 await 它就跳转，
   跳转发生时 `isLoggedIn` 仍为 true，用户会被 `LoginView` 弹回首页。修法是本地状态
   同步先清、网络调用后发。
2. **Task 13**：首次导航由 `app.use(router)` 触发而非 `mount()`，`await fetchMe()`
   排在 `app.use(router)` 之后会让已登录用户刷新时被守卫踢到登录页。
3. **Task 14**：`safeRedirect` 的控制字符绕过——浏览器会静默剥离 `\t \n \r`，
   `/login?redirect=%2F%09%2F%2Fevil.com` 解析后等价于 `//evil.com`。
4. **Task 14**：登录失败的 401 被 `http.js` 的全局拦截器截胡，后端「邮箱或密码不正确」
   被换成「登录已失效」，还把 URL 污染成 `/login?redirect=/login`，导致**输错一次密码后
   即使输对也进不去应用**。修法是区分「跳过 handler」与「这次 401 不是登录失效」两种语义。
...
实际做法是抽到 **`apps/server/src/services/db-errors.ts`**，`session.ts` 与 `auth.ts` 两处
import。计划里那段函数定义不要照抄。

### 三个已知问题（均已解决，保留原文备查）

1. ~~**Task 10 有一条覆盖损失正在修**~~（已修，`e38cebf`）。`chat.test.ts` 原有用例「数据库不可用时照常流式输出，
...
2. ~~**CLAUDE.md 的一句措辞已经过时**~~（已改，`e16c2a2`；注意真实行为是 **500** 而非 401——
   `resolveUser` 的 try/catch 只包 `verify()`，`findById()` 的错误直接冒泡到 `onError`）。现有原文是「落库失败不中断对话——
...
3. ~~**`app.ts` 里有一句注释指向还不存在的文件**~~（已对应上）。`isolation.test.ts` 里确实
   有且只有两条用例守着挂载顺序（「会话列表没有 cookie 返回 401」「对话端点没有 cookie
   返回 401」），变异验证确认调整顺序会让它们变红。
...
- **依赖方向固定** `server → database → config`，package 之间只能指向更底层。
...
| `apps/server/src/services/password.ts` | scrypt 哈希与校验，纯函数 |
| `apps/server/src/services/password.test.ts` | 同上测试 |
| `apps/server/src/services/auth.ts` | 注册/登录逻辑、登录失败限流 |
| `apps/server/src/services/auth.test.ts` | 同上测试 |
| `apps/server/src/http/middleware/auth.ts` | `resolveUser` / `requireAuth` / `requireAdmin` |
| `apps/server/src/http/middleware/auth.test.ts` | 同上测试 |
| `apps/server/src/http/routes/auth.ts` | register / login / logout / me |
| `apps/server/src/http/routes/auth.test.ts` | 同上测试 |
| `apps/server/src/http/routes/admin.ts` | 用户列表 / 禁用用户 |
| `apps/server/src/http/routes/admin.test.ts` | 同上测试 |
| `apps/server/src/http/routes/isolation.test.ts` | 跨用户越权回归测试组 |
| `apps/server/src/types.ts` | Hono context 的 `Variables` 类型声明 |
...
| `apps/server/src/services/session.ts` | 接受 `userId` 参数 |
| `apps/server/src/http/routes/sessions.ts` | 从 context 取当前用户 |
| `apps/server/src/http/routes/chat.ts` | 同上 + 403 穿透降级 |
| `apps/server/src/http/app.ts` | 挂载 auth / admin 路由与 `requireAuth` |
...
**Files:**
- Create: `apps/server/src/services/password.ts`
...
创建 `apps/server/src/services/password.test.ts`：
...
Run: `pnpm vitest run apps/server/src/services/password.test.ts`
Expected: FAIL，模块不存在

- [ ] **Step 3: 实现**

创建 `apps/server/src/services/password.ts`：
...
```bash
git add apps/server/src/services/password.ts apps/server/src/services/password.test.ts
...
创建 `apps/server/src/services/auth.test.ts`：
...
Run: `pnpm vitest run apps/server/src/services/auth.test.ts`
Expected: FAIL，模块不存在

- [ ] **Step 3: 实现**

创建 `apps/server/src/services/auth.ts`：
...
```bash
git add apps/server/src/services/auth.ts apps/server/src/services/auth.test.ts
...
- Create: `apps/server/src/http/middleware/auth.ts`
- Test: `apps/server/src/http/middleware/auth.test.ts`
...
创建 `apps/server/src/http/middleware/auth.test.ts`：
...
Run: `pnpm vitest run apps/server/src/http/middleware/auth.test.ts`
Expected: FAIL，模块不存在

- [ ] **Step 3: 建 context 类型**

创建 `apps/server/src/types.ts`：
...
创建 `apps/server/src/http/middleware/auth.ts`：
...
```bash
git add apps/server/src/types.ts apps/server/src/http/middleware/auth.ts apps/server/src/http/middleware/auth.test.ts
...
**Files:**
- Create: `apps/server/src/http/routes/auth.ts`
- Test: `apps/server/src/http/routes/auth.test.ts`
...
创建 `apps/server/src/http/routes/auth.test.ts`：
...
Run: `pnpm vitest run apps/server/src/http/routes/auth.test.ts`
Expected: FAIL，路由不存在（404）

- [ ] **Step 3: 实现路由**

创建 `apps/server/src/http/routes/auth.ts`：
...
修改 `apps/server/src/http/app.ts`：
...
```bash
git add apps/server/src/http/routes/auth.ts apps/server/src/http/routes/auth.test.ts apps/server/src/http/app.ts
...
**Files:**
- Create: `apps/server/src/http/routes/admin.ts`
- Test: `apps/server/src/http/routes/admin.test.ts`
- Modify: `apps/server/src/http/app.ts`
...
创建 `apps/server/src/http/routes/admin.test.ts`：
...
Run: `pnpm vitest run apps/server/src/http/routes/admin.test.ts`
Expected: FAIL，路由不存在

- [ ] **Step 3: 实现**

创建 `apps/server/src/http/routes/admin.ts`：
...
在 `apps/server/src/http/app.ts` 的 import 区加 `import { admin } from "./routes/admin.ts";`，
...
```bash
git add apps/server/src/http/routes/admin.ts apps/server/src/http/routes/admin.test.ts apps/server/src/http/app.ts
...
此时 `apps/server` 的测试会因为签名变化而失败——Task 10 修复。
...
- Modify: `apps/server/src/http/routes/sessions.ts`
...
修改 `apps/server/src/services/session.ts`。工厂函数签名加 `userId`，去掉 `DEFAULT_USER_ID` 的 import：
...
修改 `apps/server/src/http/routes/sessions.ts`：把 `new Hono()` 改成 `new Hono<AppEnv>()`，
...
修改 `apps/server/src/http/routes/chat.ts`。`prepareSession` 加 `userId` 参数，并让 403 穿透降级：
...
修改 `apps/server/src/http/app.ts` 的路由区：
...
1. `apps/server/src/services/session.test.ts`：`createSessionService(testDb.db)` → `createSessionService(testDb.db, DEFAULT_USER_ID)`
2. `apps/server/src/http/routes/sessions.test.ts` 与 `chat.test.ts`：所有 `app.request(...)` 需要带 cookie。在文件里加一个和 `admin.test.ts` 相同的 `registerUser` 辅助函数，`beforeEach` 里注册一个用户并把 cookie 存进变量，每个请求带上。同时 `createSessionService` 的调用要用注册出来的用户 id。
...
**Files:**
- Create: `apps/server/src/http/routes/isolation.test.ts`
...
创建 `apps/server/src/http/routes/isolation.test.ts`：
...
Run: `pnpm vitest run apps/server/src/http/routes/isolation.test.ts`
Expected: PASS，9 个用例
...
```bash
git add apps/server/src/http/routes/isolation.test.ts
...
import 从 `../schema.ts` 改到 `../testing.ts`（`apps/server` 那个改成 `@petrel/database/testing`）：
...
```bash
git add packages/database apps/server/src/services/session.test.ts
...
在「架构」一节的 `apps/server` 条目里，把路由列表改成：
