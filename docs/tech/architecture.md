...
- Composer 命令：机制在 `composables/useCommandPalette.js`，`/` 与 `Ctrl+K` 共用
  `views/ChatView.vue` 注册的命令（`/new` `/clear` `/model` `/compact` `/context`
  `/workspace` `/sidebar`）。模型选择写入账号偏好，下一条消息读取最新值。
