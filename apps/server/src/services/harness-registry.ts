...
export function createHarnessRegistry(options: HarnessRegistryOptions) {
  ...
  async function applyAssembly(entry: Entry, assembly: HarnessAssemblyOptions): Promise<void> {
    ...
    // HEU-54：常驻 harness 可能绑定 per-user Models，模型对象必须从它自己的
    // catalog 解析，不能把 global Models 的对象塞进用户作用域实例。
    const desired = resolveModel({ modelId: assembly.modelId, models: entry.harness.models });
